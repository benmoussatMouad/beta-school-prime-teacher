...
import { useGetTeachersDebts } from "api/admin/getTeachersDebts";
import PaymentDialog from "examples/Dialogs/ConfirmPayment";



function TeachersDebts() {
  const { user } = useAuth();
  ...
  const { data, isLoading } = useGetTeachersDebts({
  ...
  const { columns, rows } = teacherTableData(t, data?.debts || [], handleOpen, user);
